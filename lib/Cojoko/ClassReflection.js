define(['lodash', './Method', 'Cojoko/debug'], function (_, Method, debug) {
  ...
  return function () {
    ...
    this.normalizeInitValue = function (value, propertyName) {
      if (
        ...
        throw 'ClassReflection: The init value in property '+propertyName+' from class cannot be used. It is too complex. Remove the init value from property and set it in the init() function.';
      }
    };

    this.addMethod = function (name, body) {
      var method;
      if (_.isString(body) || _.isFunction(body)) {
        method = new Method(name, body);
        ...
        throw new Error('Parameter for addMethod have to be an instanceof Method, a string or a function');
      }
    ...
    this.isSubclassOf = function (className) {
      return this.mixins[className] !== undefined || (this.extend !== undefined && this.extend.getName() === className);
    };
