/*jshint evil:true */
define(['require', 'Cojoko/debug', 'Cojoko/Method', 'esprima', 'lodash', 'escodegen', 'text!Cojoko/Compilers/Coffee/preambel.js', 'ast-types'], function(require, debug, Method, esprima, _, escodegen, coffeePreambel) {
  ...
  return function() {
    ...
    this.compile = function(CojokoClass) {
      ...
      className = CojokoClass.getName();
      parentClassIdentifier = undefined;
      isMixin = CojokoClass.reflection.isMixin();
      ...
      _.forEach(CojokoClass.reflection.getMixins(), function (mixin) {
        var mixinIdentifier = b.identifier(mixin.getUniqueName());

        neededClasses.push(mixin);
        doCompileArgs.push(mixinIdentifier);
        mixins.push(mixinIdentifier);
      });
    ...
    this.compileProgram = function(CojokoClass) {
      ...
      if (CojokoClass.reflection.isMixin()) {
        statements.push(
          b.returnStatement(
            this.compileMixinObject(CojokoClass)
          )
        );
      } else {
        statements.push(
    ...
    this.compileClassClosure = function(CojokoClass) {
      ...
      if (mixins.length) {
        classBody.push(
          b.expressionStatement(
            b.callExpression(
              b.identifier('__mixin'),
              [classNameIdentifier, b.arrayExpression(mixins)]
            )
          )
        );
      }
      ...
      classBody = classBody.concat(
        this.compileMethods(
          CojokoClass,
          function (name, compiledMethod) {
    ...
    this.compileConstructor = function(CojokoClass) {
      ...
      // init properties to initial values
    ...
    this.compileMixinObject = function(CojokoClass) {

      var mixinProps = this.compileMethods(
        CojokoClass,
        function (name, compiledMethod) {
          return b.property('init', b.literal(name), compiledMethod);
        }
      );
    ...
    this.compileInitProperty = function(property, propsId) {
      var propsValue = b.memberExpression(propsId, b.identifier(property.name), false); // props.{[property.name]}

      var isUndefined = b.binaryExpression(
        "===",
        propsValue,
        b.identifier('undefined')
      );

      var assign = function (valueExpression) {
        return b.expressionStatement(
          b.assignmentExpression(
            '=',
      ...
      if (property.required) {
        return b.ifStatement(
          ...
          b.throwStatement(
            b.newExpression(b.identifier('Error'), [b.literal("Missing property '"+property.name+"' on construct of class: "+className+'. The property is required.')])
          ),
      ...
      } else if (property.init === undefined) { // without default
        return assign(propsValue);
        ...
        return b.ifStatement(
          ...
          assign(this.compileInitValueExpression(property.init, property)),
    ...
    this.compileInitValueExpression = function(value, property) {
      ...
      if (_.isPlainObject(value) && _.isEmpty(value)) {
        ...
        throw "Compiler: The init value in property '"+property.name+"' from class '"+className+"' cannot be compiled - it is too complex. Remove the init value from property and set it in the init() function of the class.";
      }
    };

    /**
     * @param function compile function(name)
     * @return array with return values from compile()
     */
    this.compileMethods = function(CojokoClass, compile) {
    ...
    this.generateMethods = function (CojokoClass) {
      var methods = _.clone(CojokoClass.reflection.getMethods());
      var properties = CojokoClass.reflection.getProperties();
      ...
      for (var name in properties) {
        ...
        if (property.is.match(/g/)) {
          ...
          if (!methods[getterName]) {
            ...
            methods[getterName] = new Method(getterName, func);
          }
        }
        ...
        if (property.is.match(/s/)) {
          ...
          if (!methods[setterName]) {
            func = "function (value) {\n      this." + name + " = value;\n      return this;\n    };";
            methods[setterName] = new Method(setterName, func);
          }
        }
      }
    ...
    this.compileMethod = function(method, name, CojokoClass) {
      var program;
      var methodCode = method.getBodyAsString();
