define(['lodash', 'Cojoko/debug'], function (_, debug) {
  ...
  return function () {
    ...
    this.initModuleDefinition = function (cojokoClass, options) {
      var amd = _.clone(cojokoClass.getModuleDefinition());
    ...
    this.writeDefine = function (cojokoClass, amd, options) {
      var paths = _(amd).map(function (key) { return "'"+key+"'"; } );
      var params = _(amd).keys();
