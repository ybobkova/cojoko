define(['lodash'], function (_) {
  ...
  return function () {
    ...
    this.writeClass = function (CojokoClass) {
      ...
      if (CojokoClass.name) {
        code += indent+"Cojoko.Class("+that.exportString(CojokoClass.name)+", {"+eol;
      } else {
    ...
    this.writeInitValue = function (value, name) {
      if (_.isPlainObject(value) && _.isEmpty(value)) {
        ...
        throw 'The init value in property '+name+' cannot be exported. Remove the init value and set it in the init() function in class';
      }
    };
