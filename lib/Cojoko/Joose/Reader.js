/*jshint evil:true*/
define(['joose', 'lodash', 'esprima', 'escodegen', 'Cojoko/Method', 'AMD/Dependency', 'AMD/Definition'], function (Joose, _, esprima, escodegen, CojokoMethod, Dependency, Definition) {
  ...
  return function (Cojoko) {
     ...
     * @param ClassCodeReader classCodeReader .getClassCode(string fqn), .extractClassCode(string fqn, loc)
     */
    this.read = function (jooseClassFQN, classCodeReader) {
      if (!_.isObject(classCodeReader) || !_.isFunction(classCodeReader.getClassCode) || !_.isFunction(classCodeReader.extractClassCode)) {
        throw new Error('Please provide a Cojoko/ClassCodeReader that reads the raw code for the classes');
      }
      ...
      try {
        jooseClass = this.parse(this.classCodeReader.getClassCode(jooseClassFQN));
      } catch (Ex) {
        var error = new Error('Joose/Reader: Cannot read class '+jooseClassFQN+'. '+Ex.toString());
        throw error;
      }
      ...
      var cojokoClass = Cojoko.getClassFor(compiledClass);

      that.addDependencies(cojokoClass, jooseClass);
    ...
    this.parse = function (jooseClassCode) {
      ...
      try {
        ast = esprima.parse(jooseClassCode, { raw: true, comment: true, range: true, loc: false });
      } catch (Ex) {
        throw this.parseError('e cannot parse: '+jooseClassCode+' Reason: '+Ex.toString());
      }
    ...
    this.parseClassCall = function (statements, jooseClass) {
      try {
        ...
        if (classType === 'Role') {
          ...
          throw this.parseError('only Joose.Class or Joose.Role call expressions are expected', callExpression);
        }
        ...
        if (Ex.isAssertion) {
          throw this.parseError('statements from class should be only one ExpressionStatement with CallExpression: Joose.Class. Parse Error: '+Ex.toString(), statements);
        }
    ...
    this.parseClassDefinition = function (classArguments, jooseClass) {
      if (classArguments[0].type === s.Literal) {
        jooseClass.fqn = this.assertLiteral(classArguments[0], 'class FQN');
    ...
    that.addDependencies = function (cojokoClass, jooseClass) {
      // first map all non defined paths in params to undefined and zip the object
      // paths are always more than params
      var def = jooseClass.define;

      if (def.paths.size < def.params.size) {
        throw new Error('The dependencies are misconfigured. There are more params then paths in define()');
      }

      _.each(def.paths, function (requirePath, key) {
        var alias = def.params[key]; // may be undefined
        var possibleClassName = requirePath.replace(/\//g, '.');

        var type;
        if (!cojokoClass.reflection.isSubclassOf(possibleClassName) && alias !== 'Joose') {

          if (that.isClassFQN(possibleClassName)) {
            cojokoClass.addExplicitClass(that.read(possibleClassName, that.classCodeReader));
          } else {
            cojokoClass.addDependency(requirePath, alias);
          }
        }
      });
    };
    ...
    this.parseRoles = function (doesExpression, jooseClass) {
      var role;
      ...
      if (doesExpression.type === s.MemberExpression) {
        role = this.constructFQN(doesExpression, 'role in does');
        jooseClass.roles[role.fqn] = role;
      } else if (doesExpression.type === s.ArrayExpression) {
        _.each(doesExpression.elements, function(memberExpression) {
          role = that.constructFQN(memberExpression, 'role in array from does');
          jooseClass.roles[role.fqn] = role;
        });
      } else {
        throw this.parseError('does Expression in specification has unknown type: '+doesExpression.type);
      }
    };
    ...
    this.parseInitMethod = function (afterObjectExpression, jooseClass) {
      ...
      if (jooseClass.methods['initialize']) {
        throw 'Can only read one modifier (after or before) and only initialize from Joose Class. You have to change the definition from: '+jooseClass.fqn+'.';
      }
      ...
      if (size === 1 && afterObject.initialize) {
        ...
        throw 'Can only read one method (from after or before) for initialize from Joose Class. You have to change the definition from: '+jooseClass.fqn+'.';
      }
    };
    ...
    this.parseMethod = function (functionExpression, name, jooseClass) {
      ...
      /* we want to keep all the code indentet like in joose before and manage to keep all the comments in the code
         thats why we do not codegenerate the functionExpression, we extract it from the real class with the range info
         from the esprima parser 
       */

      var js = this.classCodeReader.extractClassCode(jooseClass.fqn, functionExpression.range);
    ...
    this.specifyMixins = function(spec, jooseClass) {
      if (jooseClass.roles) {
        spec.mixins = _.map(jooseClass.roles, function (roleClass) {
          return that.read(roleClass.fqn, that.classCodeReader);
        });
      }
    };
    ...
    this.specifyExtends = function (spec, jooseClass) {
      if (jooseClass.isa) {
        spec['extends'] = this.read(jooseClass.isa.fqn, this.classCodeReader);
      }
    };
    ...
    this.specifyMethods = function (spec, jooseClass) {
      if (_.size(jooseClass.methods) > 0) {
        ...
        _.each(jooseClass.methods, function (method) {
          if (method.getName() === 'initialize') {
            method.setName('init');
          }

          method.modifyBody(function (code) {
            return code.replace(/(this|that)\.\$\$/g, '$1.');
          });

          spec.methods[method.getName()] = method;
        });
      }
    };
    ...
    this.assertProperty = function (node, msg) {
      ...
      return {
        key: this.assertIdentifierOrLiteral(node.key),
        value: node.value
      };
    };
    ...
    /**
     * @return the value of the literal or identifier
     */
    this.assertIdentifierOrLiteral = function (expression) {
      if (expression.type === s.Literal) {
        return that.assertLiteral(expression);
      } else if (expression.type === s.Identifier) {
        return that.assertIdentifier(expression);
      } else {
        throw this.parseError('expected literal or identifier but '+expression.type+' was given');
      }
    };
    ...
    this.isClassFQN = function (fqn) {
      // @FIXME: dirty hack: no other idea here
      return (/^(Psc\.|tiptoi\.|CoMun\.).+/).test(fqn);
    };
  };
});