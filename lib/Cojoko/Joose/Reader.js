/*jshint evil:true*/
define(['joose', 'lodash', 'esprima', 'escodegen', 'Cojoko/Method', 'AMD/Dependency'], function (Joose, _, esprima, escodegen, CojokoMethod, Dependency) {
  ...
  return function (Cojoko) {
    ...
    that.addDependencies = function (cojokoClass, jooseClass) {
      ...
      var amd = cojokoClass.getModuleDefinition();
      ...
      _.each(def.paths, function (requirePath, key) {
        ...
        var type;
        if (cojokoClass.reflection.isSubclassOf(possibleClassName)) {
          type = 'class.cojoko.implicit';
          alias = possibleClassName;
        } else if (that.isClassFQN(possibleClassName)) {
        ...
        if (alias !== 'Joose') {
          amd.addDependency(new Dependency(requirePath, alias, type));
        }
      });
    };
